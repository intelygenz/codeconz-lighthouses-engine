...
class PolicyCNN(nn.Module):
    ...
    def act(self, state):
        ...
        Given a state, take action
        """
        state = np.array([np.transpose(state[i], (2,0,1)) for i in range(len(state))])
...
class REINFORCE(bot.Bot):
    def __init__(self, state_maps=True, model_filename='model_save.pth', use_saved_model=True):
        super().__init__()
        self.NAME = "REINFORCE"
    ...
    def optimize_model(self, transitions):
        ...
        for i in range(max_t):
            rewards.append(transitions[i][2][0])
